                ...
                <div className="space-y-3">
                    {faqData.map((faq, i) => (
                        <motion.details
                            ...
                            className="group rounded-2xl select-none border border-emerald-400/10 bg-emerald-950/25 transition-colors"
                        >
                            <summary className="flex items-center justify-between p-4 cursor-pointer">
                                <h4 className="font-medium text-white">{faq.question}</h4>
                                <ChevronDownIcon className="w-5 h-5 text-emerald-100/70 group-open:rotate-180 transition-transform" />
                            </summary>
                            <p className="p-4 pt-0 text-sm leading-relaxed text-emerald-100/70">
                                {faq.answer}
                            </p>
                        </motion.details>
                    ))}
